...
SET ticket_url =
    (SELECT serveur FROM cas_service) 
        ...
        || '&service=' || sqlpage.protocol() || '://' || sqlpage.header('host') || '/comptes/cas/redirect_handler.sql'
        || '&format=JSON';
...
sqlpage.read_file_as_text('menu_0.json')    AS properties; 
-- Redirect the user to the home page
select 
    'steps'  as component,
    TRUE     as counter,
...
select 
    ...
    TRUE                     as active;
select 
    'Application Forum' as title,
    'briefcase' as icon;

SELECT 'button' as component,
       'center' as justify;
SELECT 
'Poursuivre vers l''application RESTAURANT' as title,
'/index.sql'as link;
